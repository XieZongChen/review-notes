...
- `<animate>`：用于在 SVG 中创建基本的属性动画。它可以用于对元素的位置、大小、颜色等属性进行动画处理。通过指定起始值和结束值，以及动画的持续时间和重复次数，可以实现属性的渐变过渡效果
- `<animateTransform>`：用于对 SVG 元素的变换属性进行动画处理，例如平移、旋转、缩放等。它可以通过指定起始值和结束值，以及动画的持续时间和重复次数，实现变换属性的平滑过渡动画
- `<animateMotion>`：用于在 SVG 中创建沿路径移动的动画效果。它可以使元素沿着指定的路径进行运动，并可以控制运动的速度、方向和重复次数。通过指定路径和动画的持续时间，可以实现元素沿路径移动的动画效果
