...
### Vue3 与 Vue2 虚拟 DOM 的差异

1. ​ 响应式系统重构 ​
   - ​Vue2​：基于 Object.defineProperty，无法监听对象属性新增/删除。
   - ​Vue3​：改用 Proxy，支持更细粒度的依赖追踪和动态属性响应。
2. ​ 虚拟 DOM 重写 ​
   - Vue2​：使用 Snabbdom 实现，依赖全量 Diff 算法。
   - ​Vue3​：彻底重写虚拟 DOM，引入以下优化：
     - 静态节点提升：编译阶段提取静态内容，避免运行时比对。
     - ​PatchFlag 标记：标记动态节点类型（如 TEXT、CLASS），加速 Diff 过程。
     - ​Fragments 支持：允许组件返回多个根节点，减少冗余包装。
3. ​ 编译器优化 ​
   - ​ 模板编译：Vue3 的编译器生成更高效的渲染函数，减少运行时开销。
...
**同层比较**​：仅比较同一层级的节点，避免跨层级遍历，将时间复杂度从 O(n³) 降低至 O(n)

**双端比较**​：同时从头部和尾部开始比较节点，减少比较次数。若首尾节点相同，则直接复用，仅移动指针

**Key 属性优化**​：通过唯一 key 标识节点，快速定位可复用的节点，避免不必要的 DOM 操作

### Vue3 Diff 策略

Vue3 在 Vue2 的基础上，增加了以下优化策略：

**PatchFlag​**：编译时为动态节点（如绑定 v-if、v-for）添加标记，更新时仅处理标记节点，减少遍历开销

**静态提升 ​**：静态节点（如纯文本、无动态绑定的元素）在编译时提取为常量，避免重复创建和比较

**块树（Block Tree）**：​​ 将动态节点和静态节点分组为块，更新时仅遍历动态块，提升性能

### Vue3 与 Vue2 Diff 策略的差异

| 特性 ​                 | ​Vue2​                     | ​Vue3​                                  |
| ---------------------- | -------------------------- | --------------------------------------- |
| ​ 静态节点处理 ​       | 每次更新均重新比较静态节点 | 编译时标记静态节点，更新时直接跳过      |
| ​ 列表 Diff 优化       | 依赖双端比较和 key 映射表  | 引入 ​ 最长递增子序列，减少移动操作     |
| ​ 动态节点标记 ​       | 无特殊标记                 | 通过 ​PatchFlag​ 标记动态节点，精准更新 |
| ​ 区块树（Block Tree） | 无                         | 将静态内容组织为区块，跳过无关节点比较  |
| ​ 剩余节点处理 ​       | 建立 key 映射表，逐个比对  | 建立索引映射表，结合最长递增子序列优化  |

### Vue3 与 Vue2 的 Diff 算法差异

| 特性 ​              | Vue2 双端比较算法 ​​         | ​Vue3 的 LIS（最长递增子序列） 优化 ​    |
| ------------------- | ---------------------------- | ---------------------------------------- |
| ​ 核心逻辑 ​        | 从首尾双向遍历，匹配相同节点 | 双端匹配 + 中间无序部分用 LIS 优化       |
| ​ 时间复杂度 ​      | O(n²)（最坏情况）            | 接近 O(n)（LIS 优化后）                  |
| ​ ​DOM 移动次数 ​ ​ | 较多（需逐个处理乱序节点）   | 更少（仅处理 LIS 之外的节点）            |
| ​ 适用场景 ​        | 简单列表或小规模数据         | 大规模动态列表（如排序、插入、删除频繁） |

### 最长递增子序列（LIS）的具体应用场景 ​

**动态列表的重新排序**

当列表元素的顺序发生变化（如插入、删除、移动节点）时，Vue3 通过 LIS 算法快速确定无需移动的最长连续子序列，仅对需要移动的节点进行操作。例如：

- 旧列表：[A, B, C, D] → 新列表：[D, A, B, C]
- LIS 为 [A, B, C]，仅需移动 D 到头部，避免全量 DOM 操作

**减少 DOM 移动次数 ​**

在 Vue2 的双端比较算法中，若列表中间节点顺序变化，需逐个比对并触发多次 DOM 移动。而 Vue3 通过 LIS 直接跳过已确定顺序的节点，仅处理剩余部分，显著降低移动次数

**结合贪心算法与二分查找优化性能 ​**

Vue3 的 LIS 实现采用贪心策略 + 二分查找 ​（时间复杂度 O(n log n)），通过维护一个最小尾部值的数组，快速确定递增序列的边界
